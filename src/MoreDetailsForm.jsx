...
export default function MoreDetailsForm({ onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!token)
        throw new Error("No token from Clerk. Check your Clerk JWT template.");
      ...
      if (typeof onSubmit === "function") {
        ...
        if (payload.digilockerLink)
          updates.digilocker_link = payload.digilockerLink;
        ...
        if (!data || data.length === 0) {
          throw new Error(
            "No profile row updated. Ensure user.id exists in Supabase."
          );
        }
